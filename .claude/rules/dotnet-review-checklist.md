...
9. **Layering respected.** The file lives in one layer and depends only downward (`Plugin -> Worker -> Devices -> Transport -> Protocol -> Logging`). Native USB calls appear only in `Transport/`'s thin `Windows*` adapters, and nothing uses HidSharp. A bounded call that makes more than one native call checks its token before each. No I/O or clock access inside a Protocol encoder. An upward dependency, a native call outside the adapters, or a compound bounded call that does not check its token is a bug.
...
11. **Minimal public surface.** Only the `IPlugin3` type is `public`. Flag any newly `public` type or member that is not the host entry point; it should be `internal`. Widening visibility to make a test compile is wrong - internals are visible to the test project via `InternalsVisibleTo`.
